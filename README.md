...
- 支持导入表情名字到 tg emoji 的映射（长按输入框的 emoji 按钮以导入），当输入框粘贴文本（必须通过长按菜单的粘贴）的时候替换为
  tg 表情（可用于映射 b 站等平台的表情文本），具体说明参见[文档](tg-custom-emoji-mapping.md)
...
- 移除主页浮动按钮
- 禁用资料头像模糊
- 在消息的时间位置显示消息 ID 和管理员头衔
- 阻止频道底部上滑切换
- 使用系统 emoji
- 反反复制，允许在禁止复制的群聊复制和保存内容，并仍然阻止转发（因为不可能成功）
- 最后上线时间精确到秒
